# -*- coding: utf-8 -*-
"""
Fallback parser for exams without instruction headers (e.g., 2024 YDS/2, 2025 YDS/1).
It looks for the first question in the group (e.g. '43') and extracts the text
preceding it on that page, or back to the end of the previous question.
"""
...
for ocr_file in EXAMS:
    ...
    for start_q, end_q in groups:
        ...
        # Find start_q
        q_pattern = re.compile(rf'(?:^|\n)\s*{start_q}\s*(?:\.|\n|Which|According|It)', re.MULTILINE)
        ...
        if not q_matches:
            ...
            if not q_matches:
                # Try finding just the number at the start of a line
                q_pattern3 = re.compile(rf'^\s*{start_q}\s*$', re.MULTILINE)
                q_matches = list(q_pattern3.finditer(full_text))
                if not q_matches:
        ...
        # Look backwards to find the end of the previous question or page marker
        search_region = full_text[:q_pos]
        
        # Find previous question number
        prev_q = start_q - 1
        ...
        # If there's an instruction header like "43-46. sorularda", start after it
        ...
        # Cloze Q16 special handling (passage is before 17)
        if len(passage) < 100 and start_q == 16:
            ...
            if q17_matches:
                ...
                instr_17 = instr_pattern.search(search_region17[marker_end_17:])
                start_17 = marker_end_17
        ...
        if len(passage) > 100:
            ...
            print(f"  Q{start_q}-{end_q}: [TOO SHORT] {len(passage)} chars")
